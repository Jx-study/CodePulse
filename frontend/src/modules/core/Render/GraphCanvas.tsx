...
export const GraphCanvas = forwardRef<GraphCanvasRef, GraphCanvasProps>(
  function GraphCanvas(
    ...
    > | null>(null);
    const posCacheRef = useRef<Map<string, { x: number; y: number }>>(
      new Map(),
    );
    ...
    useEffect(() => {
      ...
      const centroidX =
        cachedPositions.length > 0
          ? cachedPositions.reduce((s, p) => s + p.x, 0) /
            cachedPositions.length
          : width / 2;
      const centroidY =
        cachedPositions.length > 0
          ? cachedPositions.reduce((s, p) => s + p.y, 0) /
            cachedPositions.length
          : height / 2;
      ...
      simulation.on("end", () => {
        if (!svgRef.current || !zoomBehaviorRef.current || disableAutoFit)
          return;
      ...
      d3Select(svgRef.current)
        ...
        .each(function (d) {
          ...
          const propKey = isDirected
            ...
            : [d.sourceId, d.targetId].sort().join("--");
          const status = linksMap.get(propKey)?.status ?? d.status ?? "default";
          d3Select(this)
            .select("text.gc-weight-text")
      ...
      animateLink(
        ...
        if (!linkEl) {
          ...
          linkEl = d3Select(svgRef.current)
            .selectAll<SVGPathElement, GSimLink>(".gc-link")
            .filter(
              (d) => d.sourceId === elemSourceId && d.targetId === elemTargetId,
            )
            .node();
        }
