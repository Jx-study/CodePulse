...
function getNodeFill(d: GraphSimNode): string {
  return d.id === 0
    ? YOU_COLOR
    : COMMUNITY_COLORS[(d.group - 1) % COMMUNITY_COLORS.length];
}
...
const GraphOutputRenderer: React.FC<Props> = ({ data }) => {
  ...
  const simulationRef = useRef<ReturnType<
    typeof forceSimulation<GraphSimNode>
  > | null>(null);
  ...
  useEffect(() => {
    if (
      ...
      const newEntry: LeaderboardEntry = {
        ...
        elapsed,
        pathNames: gameState.path.map(
          (id) => data.nodes.find((n) => n.id === id)?.name ?? `#${id}`,
        ),
      };
      setLeaderboard((prev) => {
  ...
  useEffect(() => {
    ...
    const simulation = forceSimulation<GraphSimNode>(nodes)
      .force(
        "link",
        forceLink<GraphSimNode, GraphSimEdge>(edges)
          .id((d) => d.id)
          .distance(90)
          .strength(0.5),
      )
      ...
      .force(
        "collide",
        forceCollide<GraphSimNode>((d) => nodeRadius(d) + 6),
      );
  ...
  useEffect(() => {
    ...
    const getNodeState = (d: GraphSimNode) => {
      ...
      if (d.id === targetId && reachableIds.has(d.id))
    ...
    svg
      .selectAll<SVGCircleElement, GraphSimNode>(".nodes circle")
      .each(function (d) {
    ...
    svg
      .selectAll<SVGCircleElement, GraphSimNode>(".rings circle")
      .each(function (d) {
        const s = getNodeState(d);
    ...
    svg
      .selectAll<SVGLineElement, GraphSimEdge>(".links line")
      .each(function (d) {
        if (d.type === "recommend") return;
        const src =
          typeof d.source === "object"
            ? (d.source as GraphSimNode).id
            : (d.source as number);
        const tgt =
          typeof d.target === "object"
            ? (d.target as GraphSimNode).id
            : (d.target as number);
        const isPathEdge =
          status === "won" &&
          path.includes(src) &&
    ...
    svg
      .selectAll<SVGTextElement, GraphSimNode>(".labels text")
  ...
  const maxGroup = useMemo(
    () => data.nodes.reduce((m, n) => Math.max(m, n.group), 0),
    [data.nodes],
  );
  ...
  const isReachable = tooltip
    ? gameState.reachableIds.has(tooltip.node.id)
    : false;
