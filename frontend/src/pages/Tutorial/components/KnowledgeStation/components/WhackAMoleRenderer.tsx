...
function getDifficultyConfig(d: Difficulty): DiffConfig {
  if (d === "easy")
    return { spawnInterval: 1500, moleVisibleMs: 2500, maxConcurrent: 1 };
  if (d === "hard")
    return { spawnInterval: 600, moleVisibleMs: 1200, maxConcurrent: 3 };
      ...
      setGame((prev) => {
        ...
        if (prev.timeLeft <= 1) {
          return {
            ...
            timeLeft: 0,
            status: "gameover",
            holes: makeEmptyHoles(prev.holes.length),
          };
        }
  ...
  const renderHole = (idx: number) => {
    ...
    const holeInner = (
      ...
      >
        ...
        {mode === "2d" && (
          <span className={styles.indexLabel2d}>
            {getIndexLabel("2d", r, c)}
          </span>
        )}
      </button>
    );
    ...
    if (mode === "1d") {
      return (
        <div key={idx} className={styles.holeWrapper}>
          {holeInner}
          <span className={styles.indexLabelBelow}>
            {getIndexLabel("1d", r, c)}
          </span>
        </div>
      );
    }
