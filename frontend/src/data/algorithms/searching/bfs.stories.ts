...
【從工具到考題】
Flood Fill 是經典面試與競賽題的原型（例如「島嶼數量」「顏色填充」）。理解「連通性 + 佇列」後，也能連結到掃描線、GPU 填充等實務主題。下方小程序會用 Python 輸出圖案與尺寸等元資料，瀏覽器以 Canvas 2D 將邊線光柵為 80×80 的像素格；你在網頁上選色、點擊，實際的填色順序則由 TypeScript 端的 BFS 計算，並可用控制列播放、暫停與拖曳步驟，觀察演算法如何一格格覆寫畫面。`,
    pythonDemo: {
