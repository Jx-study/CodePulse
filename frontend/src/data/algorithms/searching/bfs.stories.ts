...
在影像編輯軟體裡，油漆桶（Fill）是最直覺的工具之一：點一個像素，同色連通區域會被新顏色取代。這背後通常不是遞迴爆堆叠，而是用佇列實作的 Flood Fill，本質就是 BFS：從起點開始，依序把四連通、且顏色符合條件的鄰居加入佇列，一層一層向外擴展，直到沒有可填的格子為止。

【為什麼是 BFS 而不是「隨便塗」】
若用深度優先（DFS）也能填滿，但 BFS 的擴展順序是「由近到遠」的同心波前，視覺上就像水波紋，也較容易平行化與除錯。多個起點同時填色時，可視為多源 BFS：每個顏色各自維持一條波前，全域時間軸上同步推進——這與本互動畫布中「多種顏色同時展開」的模型一致。
