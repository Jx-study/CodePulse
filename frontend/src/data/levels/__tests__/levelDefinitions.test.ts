...
import type { UserProgress, CategoryType } from "@/types";
import { getCategories, getNextCategory } from "@/services/CategoryService";
import { getCategoryBossLevel } from "@/services/LevelService";
...
describe("Category Helper Functions", () => {
  ...
  describe("getNextCategory", () => {
    ...
    it("should handle invalid category id gracefully", () => {
      const next = getNextCategory("invalid-category" as CategoryType);
      expect(next).toBeNull();
    });

    it("should maintain correct order sequence", () => {
      // 驗證整個順序鏈
      let current: CategoryType | null = "data-structures";
      const sequence: CategoryType[] = ["data-structures"];
