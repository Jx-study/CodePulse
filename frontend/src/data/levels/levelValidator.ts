...
export function validateLevelsData(): ValidationResult {
  ...
  for (const level of levelsData.levels as any[]) {
    ...
    if (level.prerequisites) {
      ...
      if (
        level.prerequisites.levelIds &&
        Array.isArray(level.prerequisites.levelIds)
      ) {
    ...
    if (level.pathMetadata) {
      ...
      if (
        level.pathMetadata.pathType === "portal" &&
        !level.pathMetadata.targetCategory
      ) {
        errors.push({
    ...
    if (
      level.implementationType &&
      !["algorithm", "dataStructure"].includes(level.implementationType)
    ) {
      errors.push({
  ...
  for (const [id, config] of categoryConfigs) {
    ...
    if (!categories.has(id as CategoryType)) {
      warnings.push({
